...
public class CityPickerWidget extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCityPicker.setDisplayedValues(mCurrentCitys.toArray(new String[mCurrentCitys.size()]));
        mCityPicker.setMaxValue(mCurrentCitys.size() - 1);
        mCityPicker.setValue(cityIndex);
        mCityPicker.setWrapSelectorWheel(false);

        mAreaPicker.setDisplayedValues(mCurrentAreas.toArray(new String[mCurrentAreas.size()]));
        mAreaPicker.setMaxValue(mCurrentAreas.size() > 0 ? mCurrentAreas.size() - 1 : 0);
