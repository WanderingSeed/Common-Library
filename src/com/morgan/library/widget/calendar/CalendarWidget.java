...
public class CalendarWidget extends LinearLayout {
    ...
    private void initView() {
        ...
        slideLeftIn = AnimationUtils.loadAnimation(mContext, R.anim.calendar_slide_left_in);
        slideLeftOut = AnimationUtils.loadAnimation(mContext, R.anim.calendar_slide_left_out);
        slideRightIn = AnimationUtils.loadAnimation(mContext, R.anim.calendar_slide_right_in);
        slideRightOut = AnimationUtils.loadAnimation(mContext, R.anim.calendar_slide_right_out);
        ...
        gView2.setOnItemClickListener(new OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (position < mIndexOfFirstDay) {
                    pre();
                    int preMonthPosition = gView1.selectedPositonToPos(position);
    ...
    private ArrayList<Day> initList(Date firstDayOfMonth, int f) {
        ...
        if (f > 0) {
            for (int i = 0; i < f; i++) {
                Day day = new Day(firstDayOfMonth.getTime() - (f - i) * 86400000);
                days.add(day);
            }
        }
        // 填上剩下的所有日期
    ...
    private void initList() {
        ...
        mIndexOfLastDay = mIndexOfFirstDay + getDaysOfMonth(firstDayOfMonth, lastDayOfMonth);

        int listSize = 42;
        // 如果本月第一天不是周日，在日志对象数组最前面填入上月日志对象
        if (mIndexOfFirstDay > 0) {
            for (int i = 0; i < mIndexOfFirstDay; i++) {
                Day day = new Day(firstDayOfMonth.getTime() - (mIndexOfFirstDay - i) * 86400000);
                mDays.add(day);
            }
        }
