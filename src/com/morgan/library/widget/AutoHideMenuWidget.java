...
/**
 * 一个根据当前ScrollView的滑动状态自动显示或者隐藏菜单，如果使用这个widget，必须在它内部添加id为ScrollView的{@link DirectionScrollView}
 * 和一个id为menu_container的GroupView.
 * 
 * @author Morgan.Ji
 * 
 */
public class AutoHideMenuWidget extends FrameLayout {
    ...
    private void initView()
    {
        ...
        if (null == mScrollView || null == mMenuContainer) { throw new RuntimeException(
                "must include scrollview and menu_container in this view"); }
        mScrollView.setOnTouchListener(mOnTouchListener);
        mScrollView.setOnScrollDirectionChangeListener(mScrollDirectionChangeListener);
    }

    private void initAnimatition()
    {
        mShowAnimation = new TranslateAnimation(Animation.RELATIVE_TO_SELF, 0, Animation.RELATIVE_TO_SELF, 0, Animation.RELATIVE_TO_SELF,
