	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_POINTER_DOWN:
			...
			if (spacing(event) > 10f) {
				...
				if (event.getX(1) < getWidth() && event.getX(1) > 0
						&& event.getY(1) > 0 && event.getY(1) < getHeight()) {
					mIsMulti = true;
				} else {
					mIsMulti = false;
				}
		...
		case MotionEvent.ACTION_UP:
			...
			if (getHeight() <= mParentH) {
				if (this.getTop() < 0) {
					...
					this.layout(this.getLeft(), mParentH - getHeight(),
							this.getRight(), mParentH);
				}
			} else {
				int Y1 = getTop();
		...
		case MotionEvent.ACTION_MOVE:
			if (mMode == DRAG) {
				...
				this.setPosition(mStopX - mStartX, mStopY - mStartY, mStopX
						+ this.getWidth() - mStartX,
						mStopY - mStartY + this.getHeight());
