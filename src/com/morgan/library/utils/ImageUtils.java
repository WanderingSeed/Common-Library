...
public class ImageUtils {
	...
	public static Bitmap mergeBitmap(Bitmap first, Bitmap second) {
		if (first == null || first.isRecycled() || second == null
				|| second.isRecycled()) {
			return null;
		}
		...
		Bitmap newbmp = Bitmap.createBitmap(fWidth, fHeight + sHeight,
				Config.ARGB_8888);
	...
	public static Bitmap componentBitmap(Bitmap bg, Bitmap bitmap) {
		...
		cv.drawBitmap(bitmap, (fWidth - sWidth) / 2, (fHeight - sHeight) / 2,
				null);
		// 保存
		cv.save(Canvas.ALL_SAVE_FLAG);
		// store
